...
np_hp_fit_2 <- loess(log_cost_resid_hp ~ damage_resid,
                     Truck, span = 2.0)

# Rebuild the previous plot to compare this estimate.
fig_file_name <- 'dev_np_vs_damage_dev_bw.pdf'
out_file_name <- sprintf('%s/%s', fig_dir, fig_file_name)
pdf(out_file_name)
...
np_hp_fit_3 <- loess(log_cost_resid_hp ~ damage_resid,
                     Truck, span = 0.1)
...
plot(Truck[, 'age'],
     ...
     main = 'Quadratic Model for Trucks cost',
     xlab = 'Age',
     ylab = 'Deviation of Log Trucks cost',
     col = 'blue')
...
# Generate the residuals from each model.
Truck[, 'dealer_resid'] <- lm_eng$residuals
...
# Use the loess function.
np_eng_fit_1 <- loess(log_cost_resid_eng ~ dealer_resid,
                      Truck,
                      span = 0.25)
# Calculate the predictions.
Truck[, 'dealer_np'] <- np_eng_fit_1$fitted
...
# Add a line for the linear prediction from above.
points(Truck[, 'dealer_resid'],
       predict(lm_eng_fwl),
       lwd = 3, col = 'red')

# Add a line for the nonparametric prediction.
points(Truck[, 'eng_resid'],
       np_eng_fit_1$fitted,
       lwd = 3, col = 'green')
